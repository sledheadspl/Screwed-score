      ...
      <main className="relative">
        ...
        {state.phase === 'idle' && (
          <>
            ...
            {/* ── Hero ──────────────────────────────────────────────────── */}
            <section className="max-w-4xl mx-auto px-4 sm:px-6 pt-20 pb-10 text-center relative">
              ...
              {/* Trust row */}
              <div className="animate-fade-up delay-400 flex items-center justify-center gap-6 mt-6 text-xs text-brand-sub/50">
            ...
            <div className="max-w-5xl mx-auto px-4 sm:px-6 space-y-24 pb-24">
              ...
              <section className="animate-fade-up">
                <div className="grid grid-cols-3 gap-4 sm:gap-6">
                  {[
                    { value: '$4.2M+', label: 'in overcharges found', icon: DollarSign, color: 'text-red-400' },
                    { value: '23,400+', label: 'documents analyzed',   icon: FileText,   color: 'text-brand-text' },
                    { value: '~20s',   label: 'average scan time',     icon: Zap,        color: 'text-yellow-400' },
                  ].map(({ value, label, icon: Icon, color }) => (
                    <div key={label} className="rounded-2xl border border-brand-border bg-brand-surface p-5 sm:p-7 text-center space-y-2 relative overflow-hidden"
                      style={{ boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.04)' }}>
                      <div className="absolute inset-0 bg-grid-pattern bg-grid opacity-20" />
                      <Icon className={`w-5 h-5 mx-auto ${color} relative z-10`} />
                      <p className={`text-3xl sm:text-4xl font-black relative z-10 ${color}`}>{value}</p>
                      <p className="text-xs text-brand-sub relative z-10">{label}</p>
                    </div>
                  ))}
                </div>
              </section>
